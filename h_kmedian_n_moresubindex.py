...
def h_kmedian_n(barriers, sources, targets, k, endurance, wE, wL, prepro=True, log=False, dominant=False, picture=False, time_limit=7200, init=False):
    ...
    if not(dominant):
        ...
        for e, f in vertices_source:
            for g, h in vertices_target:
                for a, b, c, d in edges_total:
                    if (a, b) in vertices_source and a == e:
        ...
        # x variables:
        # x_index = list(itertools.product(vertices_neighborhood, vertices_neighborhood))
        # print("x_index = " + str(x_index))
        ...
        for a, b, c, d, e, f, g, h in p.keys():
            if (a, b, c, d) in edges_source:
                ...
                model.addConstr(p[a, b, c, d, e, f, g, h] >= L_out * flow[a, b, c, d, e, f, g, h])
                model.addConstr(p[a, b, c, d, e, f, g, h] >= dist[a, b, c, d] - U_out * (1 - flow[a, b, c, d, e, f, g, h]))
        ...
        # model.addConstrs(p[a, b, c, d] >= flow[a, b, c, d]*dist[a, b, c, d] for a, b, c, d in edges_total)
        ...
        # Capacity constraint
        model.addConstrs(p.sum('*', '*', '*', '*', e, f, g, h) <= endurance * x[e, f, g, h] for e, f in vertices_source for g, h in vertices_target)

        model.update()

        objective = gp.quicksum(wE*p[index] for index in p_index) #+ objective

        objective += gp.quicksum(0.5*wL*flow[index] for index in flow_index)

        # for a, b, c, d in edges_barrier:
        #     # for e, f in vertices_source:
        #     #     for g, h in vertices_target:
        #     objective += dist[a, b, c, d]*flow[a, b, c, d, e, f, g, h]
        ...
        results = [len(sources), len(barriers), k, endurance, wE, wL, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]
        ...
        flow_indices = [(index, flow[index].X) for index in flow.keys() if flow[index].X > 0.5]
        # dist_indices = [(index, dist[index].X) for index in flow.keys() if flow[index].X > 0.5]
