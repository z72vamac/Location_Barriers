...
def h_kmedian_n(barriers, sources, targets, k, prepro=True, log=False, dominant=False, picture=False, time_limit=7200, init=False):
    ...
    if not(dominant):
        ...
        for a, b, c, d, e, f in paux.keys():
            ...
            model.addConstr(p[a, b, c, d] == gp.quicksum((abs(e)-1)*paux[a, b, c, d, e, f] for e, f in vertices_target))
        ...
        model.addConstrs(x[a, b, c, d] == gp.quicksum((abs(e)-1)*aux[a, b, c, d, e, f] for e, f in vertices_target) for a, b, c, d in edges_total)
