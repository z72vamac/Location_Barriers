...
%	\end{minipage}}
	...
	\CV{
		Once edges $E_X$ are represented by means of $\varepsilon$ variables. Then, some inequalities must be included to assure that the delivery can go from $P$ to $Q$ only if the segment $\segment{P}{Q}$ does not cross any barrier. These depend on the approach taken to model the paths.
	...
	\subsection{Formulations for the \KMPHN}\label{ssec:KMPHN}
	
	The formulations of the \KMPHN \ are based on the structure of the well-known $k$-Median Problem where distances between each pair of source-target neighbourhoods are represented by the shortest path joining them without traversing any barrier.
