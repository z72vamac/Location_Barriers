...
\begin{document}
	...
	\CV{
		...
		Note that, once a point is fixed in each neighbourhood, the problem that results in the induced graph $\GKMPHN$ is the $k$-median with geodesic distances, that is NP-complete.}
...
\begin{table}[h!]
	...
	\resizebox{\textwidth}{!}{%
		...
		\begin{tabular}{@{}cccl@{}}
			...
			$\epsilon_{PP'QQ'}$ & $(P,P',Q,Q')\in \mathcal D$ & $\mathbb Z_2$          & \begin{tabular}[c]{@{}l@{}}1, if  the determinants $\det(P|QQ')$ and $\det(P'|QQ')$ are both positive,\\ 0, otherwise.\end{tabular}         \\ \midrule
...
\end{table}
	...
	The goal of this subsection is to represent by linear constraints a test to check whether given two arbitrary vertices $P, P'\in \VX$, the edge $(P, P')$ belongs to $\EX$, with $X\in\{\rm KMPHN, \rm KMPN\}$, i.e., whether the line segment $\overline{PP'}$ does not intersect with any barrier of $\mathcal B$. 
	
	\CV{Given three points $P^1$, $P^2$ and $P^3$, $\determinant{P^1}{P^2}{P^3}$ denotes the following determinant:
	$$
...
%	\end{align*}}
	...
	\CV{The fact of considering the possibility of going directly from one to other neighbourhood leads to include product of continuous variables in the $\alpha$ constraints of the model that represent the determinants. Concretely, for the tuples $(P, Q, Q')\in\VB\times\EST$, the corresponding determinants produce bilinear terms. These products make the \KMPN \ to become non-convex. However, in the \KMPHN, since two of the three arguments of the determinant are fixed because $\EST$ is not considered, the $\alpha$ constraints become linear. It motivates the comparison between two formulations in terms of computational cost of considering the more general version, studied in Section \ref{section:experiments}.}
...
%\end{align*}}
	These $\varepsilon$ variables can be linearized by using the following constraints:
%	{\small
...
%	\end{equation*}}
	...
	\CV{
	\begin{equation*}\tag{$\eta$-C}\label{eq:zetaC}
...
%	The formulations of the \KMPHN \ 
	...
	%\textcolor{red}{
		...
		\begin{mini*}[2]
				...
				\addConstraint{\eqref{eq:alphaC},\eqref{eq:deltaC},\eqref{eq:varepsilonC},\eqref{eq:deltaC}, \eqref{eq:zetaC}}{}{}
...
%		\end{mini*}}
		...
		In this section, we describe a realistic application of the problem studied in this paper to perform last-mile delivery. We focus on the problem of allocating $k$ vans from where some drones depart to delivery some products to the population represented. In particular, we consider a neighbourhood in the city of Córdoba called Fátima, that is located in the northeast of the city. The sources are represented by circles that simulate zones in which vans can park. The targets are also modelled by circles that define the zones in which customers are willing to go to take the delivery. The barriers represent buildings that drones cannot cross. To simplify the problem, we assume the drones have enough endurance to go from any source to any target and they cannot ascend to avoid the buildings.
		
		We run the two models presented in Section \ref{section:formulations} in this scenario starting from the initial solution provided by the matheuristic, where the green neighbourhoods reported on the map of Fig. \ref{fig:casestudy} represent the sources in which the vans can be located, the blue, the targets where the drones serve the customers and the polygons, the buildings to be avoided. }
