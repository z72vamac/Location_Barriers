...
\begin{document}
	...
	\begin{table}[h!]
		...
		\begin{tabular}{|cl|l}
			...
			\multicolumn{1}{|c|}{$f(PQ)$} & \begin{tabular}[c]{@{}l@{}} \CV{number of times that edge $(P, Q)$ is traversed in the solution.}\end{tabular} &  \\ \cline{1-2}
	...
	\begin{align}
		\sum_{S\in\mathcal S}y(S)&=k,\tag{k-median-C1}\\
		x(ST)&\leq y(S),\quad\forall S\in\mathcal S,\quad\forall T\in\mathcal T,\tag{k-median-C2}\\
		\sum_{S\in\mathcal S} x(ST)&=1,\quad\forall T\in\mathcal T\tag{k-median-C3}.
	\end{align}
	...
	The first approach is based on a single-commodity formulation. It models a tree routed at each source that connects all targets assigned to that source. The idea is that the model must deliver one unit of commodity from the selected source neighbourhood to each of the required target neighbourhoods. Then, for each edge $(P, Q)\in E_X$, an integer variable $f(PQ)$ that counts the number of times that edge $(P, Q)$ is traversed in the solution is defined. Then, the flow conservation constraint that represents the path is the following:
	
	{\small
	\begin{equation}\tag{single-flow-C}
		\sum_{\{Q\in V_X:(P,Q)\in E_X\}}f(PQ)-\sum_{\{Q\in V_X:(Q,P)\in E_X\}}f(PQ) =\left\{
		\begin{array}{rl} 
	...
	{\small
	\begin{equation}\tag{multi-flow-C}
	...
	\subsection{Conic programming constraints}
	...
	\CV{The fact of considering the possibility of going directly from one to other neighbourhood leads to include product of continuous variables in the $\alpha$ constraints of the model that represent the determinants. These products make the \KMPN to become non-convex. However, in the \KMPHN, since two of the three arguments of the determinant are fixed, the $\alpha$ constraints become linear. It motivates the comparison between two formulations in terms of computational cost of considering the more general version, studied in Section \ref{section:experiments}.}
	...
	\begin{align*}%\tag{$\beta$-C}\label{eq:betaC}
		\betamas{P}{Q}{P_B^1}{P_B^2}&=\alphamas{P}{P_B^1}{P_B^2}\alphamas{Q}{P_B^1}{P_B^2} + \left[1-\alphamas{P}{P_B^1}{P_B^2}\right]\left[1-\alphamas{Q}{P_B^1}{P_B^2}\right],&\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB,}\\
		\betamas{P_B^1}{P_B^2}{P}{Q}&=\alphamas{P_B^1}{P}{Q}\alphamas{P_B^2}{P}{Q} + \left[1-\alphamas{P_B^1}{P}{Q}\right]\left[1-\alphamas{P_B^2}{P}{Q}\right],&\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB}.
	\end{align*}}
	...
	{\small
	\begin{align*}\tag{$\beta$-C}\label{eq:betaC}
		\betamas{P}{Q}{P_B^1}{P_B^2}&=2\gammaprod{P}{Q}{P_B^1}{P_B^2} -\alphamas{P}{P_B^1}{P_B^2}-\alphamas{Q}{P_B^1}{P_B^2}+1,&\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB,}\\
		\betamas{P_B^1}{P_B^2}{P}{Q}&=2\gammaprod{P_B^1}{P_B^2}{P}{Q} -\alphamas{P_B^1}{P}{Q}-\alphamas{P_B^2}{P}{Q}+1,&\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB.}
	\end{align*}}
	These $\gamma$ variables can be linearized by using the following constraints:
%	{\small
%	\begin{minipage}{.5\linewidth}
%		\begin{align*}
%			\gammaprod{P}{Q}{P_B^1}{P_B^2} & \leq \alphamas{P}{P_B^1}{P_B^2},\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB,}\\
%			\gammaprod{P}{Q}{P_B^1}{P_B^2} & \leq \alphamas{Q}{P_B^1}{P_B^2},\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB,}\\
%			\gammaprod{P}{Q}{P_B^1}{P_B^2} & \geq \alphamas{P}{P_B^1}{P_B^2} + \alphamas{Q}{P_B^1}{P_B^2} - 1,\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB,}
%		\end{align*}
%	\end{minipage}
%	\begin{minipage}{.5\linewidth}
%		\begin{align*}
%			\gammaprod{P_B^1}{P_B^2}{P}{Q} & \leq \alphamas{P_B^1}{P}{Q},\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB,}\\
%			\gammaprod{P_B^1}{P_B^2}{P}{Q} & \leq \alphamas{P_B^2}{P}{Q},\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB,}\\
%			\gammaprod{P_B^1}{P_B^2}{P}{Q} & \geq \alphamas{P_B^1}{P}{Q} + \alphamas{P_B^2}{P}{Q} - 1\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB,}.
%		\end{align*}
%	\end{minipage}}
		{\small
	...
	\begin{equation*}\tag{$\delta$-C}\label{eq:deltaC}
		\frac{1}{2}\left[\betamas{P}{Q}{P_B^1}{P_B^2}+\betamas{P_B^1}{P_B^2}{P}{Q}\right]\leq\deltacheck{P}{Q}{P_B^1}{P_B^2}\leq \betamas{P}{Q}{P_B^1}{P_B^2}+\betamas{P_B^1}{P_B^2}{P}{Q},\CV{\quad\forall P,Q\in V_X,\forall P_B^1P_B^2\in \VB.}
	\end{equation*}}
	...
	\begin{equation*}\tag{$\varepsilon$-C}\label{eq:varepsilonC}
		\left[\sum_{B''\in\mathcal B}\deltacheck{P}{Q}{P^1_{B''}}{P^2_{B''}}-|\mathcal B|\right] + 1\leq \varepsilonvar{P}{Q}\leq \frac{1}{|\B|}\sum_{B''\in\mathcal B}\deltacheck{P}{Q}{P^1_{B''}}{P^2_{B''}},\CV{\quad\forall P,Q\in V_X.}
	\end{equation*}
	...
	\CV{
		Once edges $E_X$ are represented by means of $\varepsilon$ variables, some inequalities must be included to assure that the delivery from $P$ to $Q$ can be produced only if the segment $\segment{P}{Q}$ does not cross any barrier. These depend on the approach taken to model the paths.
		
		For the single-commodity approach, we have:
		\begin{equation*}\tag{single-$f$-C}\label{eq:single-fC}
			f(PQ)\leq  |\mathcal T|\varepsilonvar{P}{Q},\quad\forall P, Q\in V_X.
		\end{equation*}
		...
		\begin{equation*}\tag{multi-$f$-C}\label{eq:multi-fC}
			f(PQ|ST)\leq  \varepsilonvar{P}{Q},\quad\forall (P, Q)\in E_X, \forall S\in\mathcal S, \forall T\in\mathcal T.
		\end{equation*}
...
%	The formulations of the \KMPHN \ 
	...
	%\textcolor{red}{
		...
		{\scriptsize
			\begin{mini*}[2]
				{}{\omega_E\sum_{(P,Q)\in E_X}\dvar{P}{Q}f(PQ) + \frac{\omega_L}{2}\sum_{(P,Q)\in E_X}f(PQ)}
				{}{}\label{form:H-KMPHN}\tag{H-KMPHN}
				\addConstraint{\sum_{S\in\mathcal S}y(S)}{=k}{}
				\addConstraint{x(ST)}{\leq y(S),}{\quad\forall S\in\mathcal S,\quad\forall T\in\mathcal T}
				\addConstraint{\sum_{S\in\mathcal S} x(ST)}{=1,}{\quad\forall T\in\mathcal T}
				\addConstraint{\sum_{\{Q\in V_X:(P,Q)\in E_X\}}f(PQ)-\sum_{\{Q\in V_X:(Q,P)\in E_X\}}f(PQ)}	{=\left\{
					\begin{array}{rl} 
						\sum_{T\in\mathcal T} x(ST), & \text{if } P\in S, \\
						0, & \text{if } P\in \VB, \\
						-1, & \text{if } P\in T.
					\end{array}
					\right.}{\quad\forall P\in V_X,\forall S\in\mathcal S,\forall T\in\mathcal T}
				\addConstraint{\eqref{eq:alphaC},\eqref{eq:betaC},\eqref{eq:gammaC},\eqref{eq:deltaC}}{\quad\forall P, Q\in V_X,\quad\forall P_B^1, P_B^2\in \VB}
				\addConstraint{\eqref{eq:varepsilonC}, \eqref{eq:fC}, \eqref{eq:dC}}{\quad\forall P, Q\in V_X}
				\addConstraint{\eqref{eq:nC}}{\quad\forall P\in \VS\cup \VT.}
		\end{mini*}}
